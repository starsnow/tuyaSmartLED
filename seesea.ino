...
// 270: x = width - y', y = x
uint16_t XY(uint8_t x, uint8_t y, uint8_t dir = CW0)
{
    switch (dir)
    {
    case CW0:
...
void rainbow(CRGB pLeds[])
{
    static uint8_t hue = -1;
    fill_rainbow(pLeds, NUM_LEDS_PER_MATRIX, hue++, 10);
    static int i = 0;
